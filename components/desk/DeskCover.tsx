...
export function DeskCover({ story }: { story: DeskStory }) {
  const status = [
    story.isPublished ? null : 'ЧЕРНОВИК',
    `ГЛ. ${story.ordinalCount}`,
    `${ruNum(story.words)} СЛ`,
  ]
    .filter(Boolean)
    .join(' · ');
  ...
  return (
    <Link href={`/write/${story.id}` as Route} className="group block aspect-[2/3] w-full">
      {story.gradientClass ? (
        ...
        <div className="flex h-full flex-col justify-between rounded-lg border border-border bg-surface-solid p-4 transition-colors group-hover:border-border-strong">
          <span className="font-display text-display-m italic leading-tight text-ink">
            {story.title}
          </span>
          <div>
