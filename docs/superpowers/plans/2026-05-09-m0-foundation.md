    ...
    <main className="mx-auto min-h-screen max-w-[640px] px-6 py-12 text-ink">
      ...
      <div className="mt-10 border-t border-border pt-6">
        ...
        <div className="mt-3 grid grid-cols-4 gap-3">
          ...
          <div className="aspect-square rounded-md border border-border-strong bg-bg-alt" title="bg-alt" />
        </div>
      </div>
    </main>
  );
  ...
  it('has all expected tables', async () => {
    ...
    expect(names).toEqual(
      expect.arrayContaining([
        'User', 'Story', 'Chapter', 'ChapterSummary',
        'Character', 'CharacterState', 'WorldState',
        'Tag', 'StoryTag', 'FandomCanonSeed', 'Invite',
        'Job',
      ]),
    );
  });
...
export function Marquee({
  ...
  return (
    <div className={`overflow-hidden bg-amber py-2 ${className}`}>
      <div
        data-track
        className="flex w-max animate-marquee gap-8 motion-reduce:animate-none"
      >
  ...
  "characters": [
    {
      ...
      "bible": {
        ...
        "core_traits": ["TODO founder: stubborn", "TODO founder: traumatized", "TODO founder: loyal to Foxes"],
        "canon_relationships": {
