...
vi.mock('@/lib/llm-openai', () => ({
  openaiLlm: {
    stream: async function* () { yield 'new '; yield 'text.'; },
  },
}));
...
describe('POST /api/paragraph/[id]/regen', () => {
  ...
  it('regen: streams replacement, persists text + bumps regensCount, enqueues bible', { timeout: 15_000 }, async () => {
  ...
  it('expand: inserts new paragraphs between target and next via fractional ordinals', { timeout: 15_000 }, async () => {
    const { user, chapter } = await createTestStoryWithChapter();
    const a = await prisma.paragraph.create({ data: { chapterId: chapter.id, ordinal: 1, text: 'a' } });
    ...
    llm.openaiLlm.stream = async function* () { yield 'extra one.\n\nextra two.'; };
    const res = await POST(makeReq(user.id, { mode: 'expand' }), {
      params: Promise.resolve({ id: a.id }),
    });
    expect(res.status).toBe(200);
    await res.text();
    const all = await prisma.paragraph.findMany({ where: { chapterId: chapter.id }, orderBy: { ordinal: 'asc' } });
  ...
  it('compress: merges target with next paragraph, deletes next, bumps regensCount', { timeout: 15_000 }, async () => {
    const { user, chapter } = await createTestStoryWithChapter();
    const a = await prisma.paragraph.create({ data: { chapterId: chapter.id, ordinal: 1, text: 'a' } });
    await prisma.paragraph.create({ data: { chapterId: chapter.id, ordinal: 2, text: 'b' } });
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const llm = (await import('@/lib/llm-openai')) as any;
    llm.openaiLlm.stream = async function* () { yield 'merged ab.'; };
  ...
  it('compress: last-paragraph edge case — updates text + bumps regensCount, no delete', { timeout: 15_000 }, async () => {
    const { user, chapter } = await createTestStoryWithChapter();
    const a = await prisma.paragraph.create({ data: { chapterId: chapter.id, ordinal: 1, text: 'only' } });
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const llm = (await import('@/lib/llm-openai')) as any;
    llm.openaiLlm.stream = async function* () { yield 'rewritten.'; };
    const res = await POST(makeReq(user.id, { mode: 'compress' }), {
      params: Promise.resolve({ id: a.id }),
    });
    expect(res.status).toBe(200);
  ...
  it('continue: appends new paragraphs after last paragraph with strictly increasing ordinals', { timeout: 15_000 }, async () => {
    const { user, chapter } = await createTestStoryWithChapter();
    const a = await prisma.paragraph.create({ data: { chapterId: chapter.id, ordinal: 1, text: 'start' } });
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const llm = (await import('@/lib/llm-openai')) as any;
    llm.openaiLlm.stream = async function* () { yield 'next one.\n\nnext two.\n\nnext three.'; };
