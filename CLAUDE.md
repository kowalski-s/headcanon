...
## О проекте
Headcanon — AI-платформа фанфиков с режимом просмотра историй как мини-сериалов. См. [README](README.md) и `docs/` для контекста.
...
- **Универсальные фандомы** — без ручной курации канонов командой. LLM использует знания из претрейна (модель character.ai / NovelAI). См. [architecture.md](docs/architecture.md).
- **Mobile-first, тёмная тема по умолчанию** (late-night reading).
- **Текст бесплатно, видео — премиум.** Wattpad-модель: 1% генерит, 99% потребляет.
- **Free tier для чтения обязателен.**
